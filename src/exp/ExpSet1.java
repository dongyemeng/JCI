...
public class ExpSet1 {
	...
	public static void main(String[] args) throws IOException {
		...
		// 2) plot log of # of context words of parent term vs. log of # of
		// context words of child term
		boolean taskOfParentChildPair = false;

		// Compute the # of terms for each ontology
		boolean taskOfCountNumOfTerms = false;

		// Generate labeled and unlabeled ARFF files of terms SO: 0001059 and
		// SO: 0000041
		...
		if (taskOfParentChildPair) {
			...
			// map maps a term to its sum context vector (adding all context
			// vector together)
			...
			for (String id : ids) {
				...
				ContextVector.termAndContextVectorAddition(
						termAndContextVector,
		...
		if (taskOfGenerateARFFFiles) {
			...
			for (String id : ids) {
				...
				for (TermOccurrence ocr : myArticle.occurrences) {
					if (StringUtils.equals(ocr.name, "mutation")
							|| StringUtils.equals(ocr.name, "mutations")) {
			...
			Map<String, Set<String>> nameToIDs = Utility
					.computeNameToIDs(allOccurrences);
			Map<String, List<TermOccurrence>> nameToOcrs = Utility
					.computeNameToOccrs(allOccurrences);
			Map<String, Set<String>> dupNameToIDs = getDuplicates(nameToIDs);
			Map<String, Map<String, List<TermOccurrence>>> termOccrsSharingName = Utility
					.computeTermOccrSharingName(nameToOcrs, dupNameToIDs);
			...
			String folderDir = unlabeledPapersDir;
			...
			Utility.instancesToARFFOneClass(neighborIns1, "c1", featureList,
					"data/neighbors1_train.arff");
			Utility.instancesToARFFOneClass(neighborIns2, "c2", featureList,
					"data/neighbors2_train.arff");

			Utility.instancesToARFF(mutationIns, featureList,
					"data/mutation_train.arff");
			Utility.instancesToARFF(mutationIns, featureList,
					"data/mutation_train_class1.arff", "c1", "so:0001059");
			Utility.instancesToARFF(mutationIns, featureList,
					"data/mutation_train_class2.arff", "c2", "so:0000041");
			Utility.instancesToARFF(allInstances, featureList,
					"data/mutation_unknown.arff");
		}

		if (taskOfCountVSDepthPlot) {
			String dir = CRAFTDir;
			CRAFT myCRAFT = new CRAFT(dir, ontologyName);
			List<String> ids = myCRAFT.getArticleIDs();
			int windowSize = 10;
			...
			Plotter myPlotter = new Plotter();
			myPlotter.makeScatter(
					"depth vs occurrence_" + ontologyName.toUpperCase()
							+ ".png", xData, yData, ontologyName.toUpperCase()
							+ " depth vs. # of term occurrence", "depth",
					"# of occurrence", ontologyName, false, true);
			...
			myPlotter.makePlot(
					"depth vs average occurrence_" + ontologyName.toUpperCase()
							+ ".png", xData2, yData2,
					ontologyName.toUpperCase()
							+ " depth vs. average # of term occurrence",
					"depth", "average # of occurrence", ontologyName, false,
					true);

		}
	}

	private static String findAllDupOccrs(String CRAFTDir, String ontologyName,
			int winSize, int threshold) {
		String dir = CRAFTDir;
		CRAFT myCRAFT = new CRAFT(dir, ontologyName);
		List<String> ids = myCRAFT.getArticleIDs();
	...
	private static String printStat(
			Map<String, Map<String, List<TermOccurrence>>> termOccrsSharingName,
			int threshold) {
	...
	public static Map<String, Set<String>> getDuplicates(
			Map<String, Set<String>> nameToIDs) {
